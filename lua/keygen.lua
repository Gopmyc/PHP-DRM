...
local function bWriteFile(sPath, sContent)
...
local function generateKeyPair()
    while true do
        ...
        if decrypted == bigint(104328) then
            counter = 0
            return {shared = base64.encode(tostring(n)), public = base64.encode(tostring(e))}, {shared = base64.encode(tostring(n)), private = base64.encode(tostring(d))}
        end
    end
end

if not directoryExists("key") then return error("The PHP server must ensure the creation of the folder: '/key'") end
if bExists("key/public.json") and bExists("key/private.json") then return end

local public, private = generateKeyPair()

bWriteFile("key/public.json", json.encode(public))
bWriteFile("key/private.json", json.encode(private))