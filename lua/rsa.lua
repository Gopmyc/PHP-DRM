...
local function crypt(key, number)
	if not type(key) == "table" then return "" end
	if not type(number) == "string" then return "" end
	if not (number:match("^%d+$") ~= nil) then return "" end
local exp
if key.public then
	exp = bigint(base64.decode(key.public))
else
	exp = bigint(base64.decode(key.private))
end

return tostring(modexp(bigint(number), exp, bigint(base64.decode(key.shared))))
end

if not directoryExists("key") then return error("The PHP server must ensure the creation of the folder: '/key'") end
if not (arg[1] or arg[2]) then return print("") end
if arg[1] == "decrypt" then
	if not isBase64(arg[2]) then return print("") end
	local f			=	io.open("key/private.json", "r") or error("The file: 'key/private.json' does not exist")
	local sBuffer	=	f:read("*a")
	f:close()

	local tKey = json.decode(sBuffer)
	local decryptedBytes = numberToBytes(crypt(tKey, base64.decode(arg[2])), 256, 8)
	print(bytesToString(decryptedBytes or {}))
else
	local f			=	io.open("key/public.json", "r") or error("The file: 'key/public.json' does not exist")
	local sBuffer	=	f:read("*a")
	f:close()

	local tKey = json.decode(sBuffer)
	local res = bytesToNumber(stringToBytes(arg[2]), 256, 8)
	print(base64.encode(crypt(tKey, res)))
end